...
if (!_hasitem1) exitWith {cutText [format["You need 1xItemBurlap + 1xWaterBottle"], "PLAIN DOWN"];

dayz_actionInProgress = false;

};

if (_nearanimal) exitWith {cutText [format["You're not allowed to breed more than 4 animals"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
_removed = [["ItemBurlap",1], "ItemWaterBottle"] call player_removeItems;
...
cutText [format["You need an Animal House!!"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
dayz_actionInProgress = false;
