...
_hastool = 	"ItemEtool" in _inventory;
_hasitem = [["ItemZombieParts",2], "ItemWaterBottle"] call player_checkItems;
...
cutText [format["You can't plant without Garden"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
	};

if (!_hasitem) exitWith {cutText 
[format["You need 2x ItemZombieParts + 1xItemWaterBottle"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
if (!_hastool) exitWith {cutText 
[format["You need an Etool in inventory"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["Nice! You planted some vegetables"], "PLAIN DOWN",3];	
					};
