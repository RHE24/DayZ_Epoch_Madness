...
_nearRestr = count nearestObjects [_playerPos, ["MAP_P_Stavebni_kozy"], 45] > 0;

if (_nearRestr) exitWith {
cutText [format["You can't build another Advanced Work Bench on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
	};

if (!_hasitem) exitWith {cutText 
[format["You need 4x PartWoodLumber + 1xPartWoodPlywood"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
_classname =  "MAP_P_Stavebni_kozy";    
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["Use the Advanced Work Bench to craft wood faster"], "PLAIN DOWN",3];
					};
