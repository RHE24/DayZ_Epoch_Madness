...
_hasitem = [["PartGeneric",4], "ItemPole"] call player_checkItems;
_playerPos = getPosATL player;
_nearRestr = count nearestObjects [_playerPos, ["MAP_parabola_big"], 45] > 0;

if (_nearRestr) exitWith {
cutText [format["You can't build another Antenna on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
	};

if (!_hasitem) exitWith {cutText 
[format["You need 4x Scrap Metals + 1xItemPole"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
sleep 3;
_removed = [["PartGeneric",4], "ItemPole"] call player_removeItems;	   
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					_tmpbuilt setVariable ["CharacterID",dayz_characterID,true];
						...
						cutText [format["You can pick up a frequency to gather intel on supply drops"], "PLAIN DOWN",3];	
					};
