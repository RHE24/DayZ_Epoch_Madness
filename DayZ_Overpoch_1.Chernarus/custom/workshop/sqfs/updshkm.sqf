...
_hasitem = [["ItemPole",4], ["PartGeneric",4],["50Rnd_127x107_DSHKM",3]] call player_checkItems;

_playerPos = getPosATL player;
_nearRestr = count nearestObjects [_playerPos, ["Plastic_Pole_EP1_DZ"], 27] > 0;
_nearRestr2 = count nearestObjects [_playerPos, ["DSHKM_Gue"], 45] > 0;
...
dayz_actionInProgress = false;
cutText [format["You can't build another DSHKM on an existing plot pole"], "PLAIN DOWN"];
};

if (!_hasitem) exitWith {cutText 
[format["You need 4x ItemPole + x4PartGeneric +3x50Rnd_127x107_DSHKM"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
sleep 3;
_removed = [["ItemPole",4], ["PartGeneric",4],["50Rnd_127x107_DSHKM",3]] call player_removeItems;	 
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["YOU'RE LIKE THE NEW COMMUNIST RAMBO!"], "PLAIN DOWN",3];
					};
