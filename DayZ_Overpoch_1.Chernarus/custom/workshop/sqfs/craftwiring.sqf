...
_hasitem = [["ItemPole",2], "ItemWire","ItemGenerator"] call player_checkItems;

_playerPos = getPosATL player;
_nearRestr = count nearestObjects [_playerPos, ["Plastic_Pole_EP1_DZ"], 40] > 0;
if !(_nearRestr) exitWith {
dayz_actionInProgress = false;
cutText [format["You need to be within your plot pole range"], "PLAIN DOWN"];
};

if (!_hasitem) exitWith {cutText 
[format["You need 2x ItemPole + x1ItemWire +1xGenerator in inventory"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
_classname =  "MAP_plot_green_draty";    
...
[] execVM 'custom\workshop\hpp\wiringmenu.sqf';
...
sleep 3;
_removed = [["ItemPole",2], "ItemWire"] call player_removeItems;	 
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["ELECTRIFIED FENCES"], "PLAIN DOWN",3];
					};
