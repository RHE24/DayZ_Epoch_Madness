...
_hasitem = [["PartWoodLumber",2], "ItemWire"] call player_checkItems;
...
[format["You need 2x PartWoodLumber + x1ItemWire"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
_classname =  "MAP_plot_provizorni";    
...
[] execVM 'custom\workshop\hpp\barbedwiremenu.sqf';
...
sleep 3;
_removed = [["PartWoodLumber",2], "ItemWire"] call player_removeItems;	   
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["Don't you feel safer already?"], "PLAIN DOWN",3];
					};
