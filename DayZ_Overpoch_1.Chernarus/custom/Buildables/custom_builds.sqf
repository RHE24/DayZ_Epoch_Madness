...
if (_isWater) exitWith {dayz_actionInProgress = false; cutText [localize "str_player_26", "PLAIN DOWN"];};
if (_inVehicle) exitWith {dayz_actionInProgress = false; cutText [(localize "str_epoch_player_42"), "PLAIN DOWN"];};
...
if((_requireplot>0) and ((_IsNearPlot==0)||(!_canBuildOnPlot))) exitWith {dayz_actionInProgress = false; cutText ["Building of this item requires a plot pole within range!" , "PLAIN DOWN"]; };
