...

{
	...
	*/
		...
		//portables
		_isportables = _type in DZE_portables;
		//workshop
		...
		if (!_isDZ_Buildable && !_isTrapItem) then {
			...
			// if( (count _inventory > 0) && !_isPlot && !_doorLocked) then { Edited for workshop
			if( (count _inventory > 0) && !_isPlot && !_doorLocked && !_isworkshop && !_isportables) then {
