...
"_allowed","_hasAccess","_uid","_hasKeyKit","_isLocked","_playerPos","_near"];
...
if ((_cursorTarget isKindOf "GUE_WarfareBFieldhHospital") && {speed player <= 1}) then {
if (s_player_hospitalhero < 0) then {
    s_player_hospitalhero = player addAction ["<t color='#ff5200'>HEAL IN TENT</t>", "custom\workshop\sqfs\healhospital.sqf", [], 5, false];
    };
    } else {
...
if ((_cursorTarget isKindOf "MAP_kulna") && {speed player <= 1}) then {
if (s_player_managework < 0) then {
    s_player_managework = player addAction ["<t color='#66CCFF'>MANAGE WORKSHOP</t>", "custom\workshop\hpp\workshopmenu.sqf", [], 5, false];
    };
    } else {
	...
	/////////////END WORKSHOP
/* // Old Replaced code for workshop
	...
	*/
///////PORTABLES 
if ((_cursorTarget isKindOf "Stove") && {speed player <= 1}) then {
if (s_player_removekiln < 0) then {
    s_player_removekiln = player addAction ["<t color='#FF0000'>Remove Kiln</t>", "custom\portables\removekiln.sqf", [], 5, false];
    };
if (s_player_managekiln < 0) then {
    s_player_managekiln = player addAction ["<t color='#FFB85F'>Manage Kiln</t>", "custom\portables\claykilnmenu.sqf", [], 5, false];
    };	
	
    } else {
    player removeAction s_player_removekiln;
    s_player_removekiln = -1;
	player removeAction s_player_managekiln;
    s_player_managekiln = -1;
    };

if ((_cursorTarget isKindOf "MAP_AirCondition_A") && {speed player <= 1}) then {
if (s_player_removesaw < 0) then {
    s_player_removesaw = player addAction ["<t color='#FF0000'>Remove Table Saw</t>", "custom\portables\removetablesaw.sqf", [], 5, false];
    };
    if (s_player_managesaw < 0) then {
    s_player_managesaw = player addAction ["<t color='#FFB85F'>Manage Table Saw</t>", "custom\portables\sawmenu.sqf", [], 5, false];
    };	
	
    } else {
    player removeAction s_player_removesaw;
    s_player_removesaw = -1;
	player removeAction s_player_managesaw;
    s_player_managesaw = -1;
    };
	
if ((_cursorTarget isKindOf "MAP_AirCondition_B") && {speed player <= 1}) then {
if (s_player_removeengineering < 0) then {
    s_player_removeengineering = player addAction ["<t color='#FF0000'>Remove Forge</t>", "custom\portables\removeforge.sqf", [], 5, false];
    };	
if (s_player_manageengineering < 0) then {
    s_player_manageengineering = player addAction ["<t color='#FFB85F'>Manage Forge</t>", "custom\portables\forgemenu.sqf", [], 5, false];
    };	
} else {
    player removeAction s_player_removeengineering;
    s_player_removeengineering = -1;
	player removeAction s_player_manageengineering;
    s_player_manageengineering = -1;
    };

if ((_cursorTarget isKindOf "MAP_Camo_Box") && {speed player <= 1}) then {
if (s_player_removecammo < 0) then {
    s_player_removecammo = player addAction ["<t color='#FF0000'>Remove CammoBox</t>", "custom\portables\removecammobox.sqf", [], 5, false];
    };	
} else {
    player removeAction s_player_removecammo;
    s_player_removecammo = -1;
    };
	
if ((_cursorTarget isKindOf "MAP_tent_small_west") && {speed player <= 1}) then {
if (s_player_removemtent < 0) then {
    s_player_removemtent = player addAction ["<t color='#FF0000'>Remove Tent</t>", "custom\portables\removemilitarytent.sqf", [], 5, false];
    };
} else {
    player removeAction s_player_removemtent;
...
if((_isbong) && _canDo) then { if (s_player_pipe < 0) then {
