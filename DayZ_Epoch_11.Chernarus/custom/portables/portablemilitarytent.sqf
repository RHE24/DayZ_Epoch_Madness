...
//portables by juandayz writed with base on modular_build.sqf
private ["_playerPos","_nearRestr","_hastool","_hasitem","_removed","_abort","_reason","_movedist","_isNear","_lockable","_isAllowedUnderGround","_offset","_classname","_zheightdirection","_zheightchanged","_rotate","_objectHelperPos","_objectHelperDir","_objHDiff","_position","_isOk","_dir","_vector","_cancel","_location2","_buildOffset","_location","_limit","_started","_finished","_animState","_isMedic","_proceed","_counter","_dis","_sfx","_combination_1_Display","_combination_1","_combination_2","_combination_3","_combination","_combinationDisplay","_combination_4","_num_removed","_tmpbuilt","_vUp","_classnametmp","_text","_ghost","_objectHelper","_location1","_object","_helperColor","_canDo","_pos","_onLadder","_vehicle","_inVehicle","_needNear","_canBuild"];
...
_nearRestr2 = count nearestObjects [_playerPos, ["MAP_tent_small_west"], 145] > 0;
 

if (_nearRestr) exitWith {
cutText [format["You cannot build  in a plot area"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
    };

if (_nearRestr2) exitWith {
cutText [format["You cannot build another Portable Cley Kiln in this area 145mts"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
    };
...
_classname =  "MAP_tent_small_west";    
...
if((count _offset) <= 0) then {
	...
	while {_isOk} do {
		...
		if(_location1 distance _location2 > _movedist) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = format[localize "STR_EPOCH_BUILD_FAIL_MOVED",_movedist];
			detach _object;
			deleteVehicle _object;
			detach _objectHelper;
			deleteVehicle _objectHelper;
		...
		if(_location1 distance _objectHelperPos > _movedist) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = format[localize "STR_EPOCH_BUILD_FAIL_TOO_FAR",_movedist];
			detach _object;
			deleteVehicle _object;
			detach _objectHelper;
			deleteVehicle _objectHelper;
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					_tmpbuilt setVariable ["CharacterID",dayz_characterID,true];
						...
						cutText [format["A Small military tent"], "PLAIN DOWN",3];	
					};
