...
_hasitem = [["PartWoodLumber",4], "PartWoodPile"] call player_checkItems;
_playerPos = getPosATL player;
_nearRestr = count nearestObjects [_playerPos, ["MAP_Misc_Boogieman"], 45] > 0;
 
if (_nearRestr) exitWith {
cutText [format["You can't build another Garden on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
	};

if (!_hasitem) exitWith {cutText 
[format["You need 4x PartWoodLumber + 1xPartWoodPile"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
if (!_hastool) exitWith {cutText 
[format["You need a Machete in your inventory"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
sleep 3;
_removed = [["PartWoodLumber",4], "PartWoodPile"] call player_removeItems; 
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["Use the Garden to plant and harvest your very own food supply!"], "PLAIN DOWN",3];	
					};
