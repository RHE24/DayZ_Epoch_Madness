...
_nearRestr = count nearestObjects [_playerPos, ["MAP_Shed_W01"], 45] > 0;
 
 
if (_nearRestr) exitWith {
cutText [format["You can't build another Animal House on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
	};

if (!_hasitem) exitWith {cutText 
[format["You need 4x woodlumbers + 4xPlywood"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["Use the Animal House to breeding animals"], "PLAIN DOWN",3];	
					};
