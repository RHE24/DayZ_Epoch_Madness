...
_hasitem = [["PartWoodLumber",4], "ItemPole"] call player_checkItems;
_playerPos = getPosATL player;
_nearRestr = count nearestObjects [_playerPos, ["Loudspeakers_EP1"], 45] > 0;

if (_nearRestr) exitWith {
cutText [format["You can't build another loudspeaker on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
    };

if (!_hasitem) exitWith {cutText
[format["You need 4x PartWoodLumber + 1xItemPole"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
sleep 3;
_removed = [["PartWoodLumber",4], "ItemPole"] call player_removeItems;   
    cutText [format[localize "str_build_01",_text], "PLAIN DOWN"];
		...
		if (DZE_StaticConstructionCount > 0) then { //if count is manually overridden inside init.sqf, use that instead, else use limits configured in config files
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				cutText [format["Use the Loudspeaker to sound the alarm!"], "PLAIN DOWN",3];	
					};
