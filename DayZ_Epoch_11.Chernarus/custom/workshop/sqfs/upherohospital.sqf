...
private ["_Playersurvived","_playerPos","_nearRestr","_hastool","_hasitem","_removed","_abort","_reason","_distance","_isNear","_lockable","_isAllowedUnderGround","_offset","_classname","_zheightdirection","_zheightchanged","_rotate","_objectHelperPos","_objectHelperDir","_objHDiff","_position","_isOk","_dir","_vector","_cancel","_location2","_buildOffset","_location","_limit","_started","_finished","_animState","_isMedic","_proceed","_counter","_dis","_sfx","_combination_1_Display","_combination_1","_combination_2","_combination_3","_combination","_combinationDisplay","_combination_4","_num_removed","_tmpbuilt","_vUp","_classnametmp","_text","_ghost","_objectHelper","_location1","_object","_helperColor","_canDo","_pos","_onLadder","_vehicle","_inVehicle","_needNear","_canBuild"];
...
_hasitem = [["cinder_wall_kit",2], "ItemWoodFloor"] call player_checkItems;
_playerPos = getPosATL player;
_nearRestr = count nearestObjects [_playerPos, ["GUE_WarfareBFieldhHospital"], 45] > 0;
...
cutText [format["Can't yet! You've only survived 1 day. You'll need to survive 2 full days",_Playersurvived], "PLAIN DOWN"]; 
dayz_actionInProgress = false;
};

 
if (_nearRestr) exitWith {
cutText [format["You can't build another Hospital on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
	};

if (PlayerHumanity < 5000) exitWith {
cutText [format["You need to be a hero"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};	
	
if (!_hasitem) exitWith {cutText 
[format["You need 2xCinderWalls + 1xWoodFloor"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					_tmpbuilt setVariable ["CharacterID",dayz_characterID,true];
						...
						cutText [format["Now you get to heal in your very own Hero tent!"], "PLAIN DOWN",3];
					};
				
			} else { //if magazine was not removed, cancel publish
				deleteVehicle _tmpbuilt;
				cutText [format["Cancel action. You are too far from your plot pole"], "PLAIN DOWN"];
			};
