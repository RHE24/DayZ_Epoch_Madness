...
_nearRestr = count nearestObjects [_playerPos, ["MAP_office_table_a"], 45] > 0;

if (_nearRestr) exitWith {
cutText [format["You can't build another Dresser on an existing plot pole"], "PLAIN DOWN"];  
			...
			if (isNumber (configFile >> "CfgVehicles" >> _classname >> "constructioncount")) then {
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					cutText [format["Use the Dresser to create your own clothes"], "PLAIN DOWN",3];
					};
