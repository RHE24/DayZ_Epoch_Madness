...
_hasitem = [["ItemSandbag",4], ["PartWoodLumber",4]] call player_checkItems;
...
cutText [format["You need to be within your plot pole area"], "PLAIN DOWN"];
};


if (!_hasitem) exitWith {cutText
[format["You need 4x SandBags + x4WoodLumbers"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
_classname =  "MAP_Pristresek_mensi";    
...
_playerPos = getPosATL player;
_nearRestr = count nearestObjects [_playerPos, ["Plastic_Pole_EP1_DZ"], 34] > 0;
...
//////////////////////////////////////////////////##########START TO BUILD AND REMOVE OBJECT
player playActionNow "Medic";
[player,"repair",0,false,10] call dayz_zombieSpeak;
[player,10,true,(getPosATL player)] spawn player_alertZombies;
sleep 3;
_removed = [["ItemSandbag",4], ["PartWoodLumber",4]] call player_removeItems;    
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					_tmpbuilt setVariable ["CharacterID",dayz_characterID,true];
						...
						cutText [format["This will do for now..."], "PLAIN DOWN",3];
					};
				
			} else { //if magazine was not removed, cancel publish
				deleteVehicle _tmpbuilt;
				cutText [format["Cancel action. Too far from your plot pole"], "PLAIN DOWN"];
			};
