...
_hasitem = [["cinder_wall_kit",4], "ItemWoodFloor"] call player_checkItems;
...
if (_Playersurvived < 6) exitWith {
cutText [format["Can't yet! You've only survived %1 days.You'll need to survive 6 full days",_Playersurvived], "PLAIN DOWN"]; 
dayz_actionInProgress = false;
};

 
if (_nearRestr) exitWith {
cutText [format["You can't build another House on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
	};

if (!_hasitem) exitWith {cutText 
[format["Needs 4xCinderWalls + 1xWoodFloor"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
...
sleep 3;
_removed = [["cinder_wall_kit",4], "ItemWoodFloor"] call player_removeItems; 
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					_tmpbuilt setVariable ["CharacterID",dayz_characterID,true];
						...
						cutText [format["You've built youself a dang mansion! Nice!"], "PLAIN DOWN",3];
					};
				
			} else { //if magazine was not removed, cancel publish
				deleteVehicle _tmpbuilt;
				cutText [format["Cancel action. You're too far from your plot pole"], "PLAIN DOWN"];
			};
