...
_nearRestr = count nearestObjects [_playerPos, ["Land_Misc_Well_L_EP1"], 45] > 0;
 
 

if (_nearRestr) exitWith {
cutText [format["You can't build another Water Tank on an existing plot pole"], "PLAIN DOWN"];  
    dayz_actionInProgress = false;
    };

if (!_hasitem) exitWith {cutText
[format["You need 2x CinderBlocks + 1xPartWoodPlywood"], "PLAIN DOWN"];
dayz_actionInProgress = false;
};
				...
				_tmpbuilt setVariable ["OEMPos",_location,true]; //store original location as a variable
				 ...
				 //if not lockable item
					...
					_tmpbuilt setVariable ["CharacterID",dayz_characterID,true];
						...
						cutText [format["Use the water tank to get fresh water"], "PLAIN DOWN",3];	
					};
